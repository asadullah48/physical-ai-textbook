    ...
    return (
        <Card className="w-full max-w-md mx-auto">
            ...
            <CardContent className="space-y-6">
                <canvas ref={canvasRef} width={300} height={300} className="bg-white rounded shadow-sm border" />
                <div className="space-y-4">
                    <div>
                        <label className="text-sm font-medium">Joint 1 (θ1): {theta1}°</label>
                        <input type="range" min="-90" max="180" value={theta1} onChange={(e) => setTheta1(Number(e.target.value))} className="w-full mt-2" />
                    </div>
                    <div>
                        <label className="text-sm font-medium">Joint 2 (θ2): {theta2}°</label>
                        <input type="range" min="-135" max="135" value={theta2} onChange={(e) => setTheta2(Number(e.target.value))} className="w-full mt-2" />
                    </div>
                    <div className="p-3 bg-blue-50 text-blue-800 text-xs rounded-md">
                        <strong>Forward Kinematics:</strong> Moving sliders calculates end-effector position from joint angles.
                    </div>
                </div>
            </CardContent>
        </Card>
